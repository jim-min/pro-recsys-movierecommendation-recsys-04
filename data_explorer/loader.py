...
@st.cache_data
def load_item_attributes(data_dir: str) -> Dict[str, Any]:
    """
    Loads Ml_item2attributes.json

    Returns:
        Dictionary mapping item_id (as string) to list of genre IDs
        Example: {"1": [8,12,13,5,9], "2": [8,13,9], ...}
    """
    file_path = os.path.join(data_dir, "Ml_item2attributes.json")

    if not os.path.exists(file_path):
        st.warning(f"Item attributes file not found: {file_path}")
        return {}
    ...
    try:
        ...
        st.success(f"✓ Loaded attributes for {len(item_attrs):,} items")
        return item_attrs

    except Exception as e:
        st.error(f"Error loading item attributes: {e}")
        return {}
